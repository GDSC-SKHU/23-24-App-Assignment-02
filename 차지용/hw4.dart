...
void main() async { //여기서도 비동기 처리를 해야겠죠?
  try{
    var user = await User("123", "person1").fetchUserById('123'); //id 값이 일치하는 경우
    print("${user.name}"); //이름이 출력된다.

    user = await User("999","person2").fetchUserById('999'); //id 값이 일치하지 않는 경우 오류 발생
    print("${user.name}");
  }
  catch(err){
    print("오류 발생: $err");
